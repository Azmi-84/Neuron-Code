...
def run_advanced_material_test():
    ...
    # Use a 2D plot for the microstructure visualization instead of 3D
    ax6 = fig.add_subplot(gs[2, :])
    ax6.set_xlabel("X")
    ax6.set_ylabel("Y")
    ax6.set_title("Microstructure Visualization (2D Grain Deformation)")
    ...
    # Lines for fatigue plots - fixing to extract the line object
    fatigue_lines = [
    ...
    micro_image = None  # Will hold the image object for microstructure

    # Generate initial 2D microstructures (grain structures for each material)
    def generate_microstructure(material_idx):
        nonlocal micro_image

        # Remove previous microstructure if it exists
        if micro_image is not None:
            try:
                micro_image.remove()
            except:
                pass  # Silently handle any issues with removing the old image

        material = materials[material_idx]

        # Create a 2D grid representing the material's microstructure
        grain_size = 0.2  # Base grain size
        grid_size = 100

        # Generate random grain centers for 2D visualization
        grain_count = 20
        centers = np.random.rand(grain_count, 2) * grid_size
        ...
        # Deform based on material strain - stretch in X direction
        strain_factor = 1.0 + material.strain * 5  # Exaggerate for visibility
        X_deformed = X * strain_factor

        # Assign each point to nearest grain and give it a value
        for i, (cx, cy) in enumerate(centers):
            distances = np.sqrt((X_deformed - cx) ** 2 + (Y - cy) ** 2)
            mask = np.exp(-distances / grain_size)
            # Add grain with a unique value
            grid += mask * (i + 1)

        # Normalize grid for coloring
        grid = grid / np.max(grid) if np.max(grid) > 0 else grid

        # Create colormap based on material color
        base_color = np.array(plt.cm.colors.to_rgba(material.color))
        color_array = plt.cm.viridis(np.linspace(0, 1, 256))
        # Blend with material color
        for i in range(256):
            color_array[i] = 0.5 * color_array[i] + 0.5 * base_color
        custom_cmap = plt.cm.colors.ListedColormap(color_array)

        # Display the microstructure
        micro_image = ax6.imshow(
            grid,
            cmap=custom_cmap,
            interpolation="nearest",
            aspect="auto",
            extent=[0, grid_size * strain_factor, 0, grid_size],
        )

        # Add title with material name
        ax6.set_title(
            f"Microstructure: {material.material_type} (Strain: {material.strain:.4f})"
        )
    ...
    def init():
        ...
        micro_objects = generate_microstructure(highlighted_material)
    ...
    def animate(i):
        nonlocal current_phase, phase_start_time, highlighted_material, notifications, micro_image
        ...
        if i % 20 == 0:
            ...
            micro_objects = [micro_image] if micro_image is not None else []
