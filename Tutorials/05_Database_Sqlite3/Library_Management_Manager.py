...
def see_catalog():
    try:
        ...
        for row in books:
            print(
                f"Book ID: {row[0]} | Book name: {row[1]} | Author: {row[2]} | Price: ${row[3]}"
            )
...
def search_book():
    ...
    try:
        book = cursor.execute(
            "SELECT * FROM catalog WHERE book_name LIKE ?", ["%" + book_name + "%"]
        ).fetchone()

        if book is not None:
            print(
                f"Book ID: {book[0]} | Book name: {book[1]} | Author: {book[2]} | Price: ${book[3]}"
            )
...
def issue_book(book_id=None):
    if book_id is None:
        ...
        book = cursor.execute(
            "SELECT * FROM catalog WHERE unique_id=?", [book_id]
        ).fetchone()
        if not book:
    ...
    try:
        student = cursor.execute(
            "SELECT * FROM user_information WHERE student_id=?", [student_id]
        ).fetchone()
        if not student:
        ...
        existing = cursor.execute(
            "SELECT * FROM issue_book WHERE student_id=? AND book_id=?",
            [student_id, book_id],
        ).fetchone()
        if existing:
        ...
        cursor.execute(
            "INSERT INTO issue_book (student_id, book_id, username, issue_date, return_date) VALUES (?, ?, ?, ?, ?)",
            (student_id, book_id, username, issue_date, return_date),
        )
        conn.commit()
...
""",
        ...
        if issued_books:
            for book in issued_books:
                print(
                    f"Issue ID: {book[0]} | Book Name: {book[1]} | Issue Date: {book[2]} | Return Date: {book[3]}"
                )

            issue_id = input("Enter the issue ID to return the book: ")
            issue = cursor.execute(
                "SELECT * FROM issue_book WHERE issue_id=? AND student_id=?",
                [issue_id, student_id],
            ).fetchone()
            if issue:
...
def add_book():
    try:
        ...
        cursor.execute(
            "INSERT INTO catalog (book_name, author, price) VALUES (?, ?, ?)",
...
def remove_book():
    try:
        ...
        issued_book = cursor.execute(
            "SELECT * FROM issue_book WHERE book_id=?", [book_id]
        ).fetchone()
        if issued_book:
            ...
            case "4":
                try:
                    ...
                    if books:
                        for book in books:
                            print(
                                f"Student ID: {book[0]} | Username: {book[1]} | Issue Date: {book[2]} | Return Date: {book[3]}"
                            )
                    else:
...
def sign_up():
    try:
        ...
        existing = cursor.execute(
            "SELECT * FROM user_information WHERE student_id=? OR email=?",
            [student_id, email],
        ).fetchone()
        if existing:
        ...
        cursor.execute(
            ...
            (student_id, username, email, password, phone_num),
        )
        conn.commit()
