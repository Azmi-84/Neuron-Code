...
def register_student():
    try:
        ...
        if not all(key in data for key in ['student_id', 'username', 'email', 'password', 'phone_num']):
...
def register_librarian():
    try:
        ...
        # Validate required fields
        if not all(key in data for key in ['librarian_id', 'username', 'email', 'password', 'phone_num']):
            return jsonify({"success": False, "message": "Missing required fields"}), 400
        ...
        if librarian_collection.find_one({"librarian_id": data['librarian_id']}):
            return jsonify({"success": False, "message": "Librarian ID already registered"}), 400
...
def add_book():
    try:
        ...
        # Validate required fields
        required_fields = ['book_name', 'author', 'price', 'quantity', 'publication', 'publication_date']
        ...
        if not data['publication'].strip():
            return jsonify({"success": False, "message": "Publication cannot be empty"}), 400
            
        if not validate_publication_date(data['publication_date']):
            return jsonify({"success": False, "message": "Invalid publication date"}), 400
...
def update_book(book_id):
    try:
        ...
        if 'publication_date' in data and validate_publication_date(data['publication_date']):
...
def issue_book(book_id):
    try:
        ...
        if not student:
            return jsonify({"success": False, "message": "Invalid student credentials"}), 400
            
        # Check if book is already issued to this student
        for issue in book.get('issued_to', []):
            if issue['student_id'] == data['student_id']:
                return jsonify({"success": False, "message": "Book already issued to this student"}), 400
...
def return_book(book_id):
    try:
        ...
        if not book:
            return jsonify({"success": False, "message": "No issued record found for this book and student"}), 404
...
def get_all_issued_books():
    try:
        ...
        issued_books = list(book_collection.find({"issued_to": {"$exists": True, "$ne": []}}))
        ...
        return jsonify({"success": False, "message": "Failed to fetch issued books"}), 500
...
def get_student_issued_books(student_id):
    try:
        ...
        for book in books:
            ...
            issue_record = next((issue for issue in book['issued_to'] 
                           if issue['student_id'] == student_id), None)
