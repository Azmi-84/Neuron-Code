...
function initializeNavigation() {
    ...
    $('#librarian-btn').addEventListener('click', () => showAuthSection('librarian'));
    ...
    ['student', 'librarian'].forEach(role => {
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                // Hide all forms
                $$(`#${role}-auth .auth-form`).forEach(form => form.classList.add('hidden'));
...
function initializeDashboardOptions() {
    ...
    $$('#student-dashboard .dashboard-option').forEach(option => {
        option.addEventListener('click', () => handleDashboardAction('student', option.getAttribute('data-action')));
    });
    
    // Librarian dashboard options
    $$('#librarian-dashboard .dashboard-option').forEach(option => {
        option.addEventListener('click', () => handleDashboardAction('librarian', option.getAttribute('data-action')));
    });
}
...
function initializeAuthForms() {
    ...
    $('#student-register-form').addEventListener('submit', (e) => {
        ...
        registerUser('student', { student_id: studentId, username, email, password, phone_num: phone });
    });
    ...
    $('#librarian-register-form').addEventListener('submit', (e) => {
        ...
        registerUser('librarian', { librarian_id: librarianId, username, email, password, phone_num: phone });
    });
...
function loadIssueBookInterface(container) {
    ...
    container.querySelector('#issue-book-form').addEventListener('submit', (e) => {
        e.preventDefault();
...
function loadReturnBookInterface(container) {
    ...
    // Initialize form submission
    container.querySelector('#return-book-form').addEventListener('submit', (e) => {
        e.preventDefault();
    ...
    if (state.currentRole === 'student' && state.currentUser) {
        fetchIssuedBooks(state.currentUser.student_id).then(issuedBooks => {
            const issuedBooksContainer = container.querySelector('.issued-books-container');
...
function loadRemoveBookInterface(container) {
    ...
    // Initialize form submission
    container.querySelector('#remove-book-form').addEventListener('submit', (e) => {
        e.preventDefault();
...
function loadUpdateBookInterface(container) {
    ...
    container.querySelector('#load-book-details').addEventListener('click', () => {
        ...
        fetchBookDetails(bookId).then(book => {
            ...
            container.querySelector('#update-book-name').value = book.book_name || '';
            ...
            container.querySelector('#update-publication').value = book.publication || '';
            container.querySelector('#update-pub-date').value = book.publication_date || '';
        });
    });
    
    // Initialize form submission
    container.querySelector('#update-book-form').addEventListener('submit', (e) => {
        e.preventDefault();
...
function loadResetPasswordInterface(container, role) {
    ...
    // Initialize form
    container.querySelector('#dashboard-reset-form').addEventListener('submit', (e) => {
        e.preventDefault();
        
        const currentPassword = container.querySelector('#dashboard-reset-current').value;
        const newPassword = container.querySelector('#dashboard-reset-new').value;
        const confirmPassword = container.querySelector('#dashboard-reset-confirm').value;
        ...
        resetPassword(role, state.currentUser.email, currentPassword, newPassword);
    });
}
...
function displayBooks(books, container) {
    ...
    books.forEach(book => {
        ...
        template.querySelector('.book-pub-date').textContent = book.publication_date;
...
async function addBook(bookData) {
    try {
        ...
        const data = await response.json();
        showNotification(`Book added successfully. Book ID: ${data.book_id}`, 'success');
...
async function updateBook(bookId, updates) {
    try {
        ...
        if ($('.catalog-container') && !$('.catalog-container').classList.contains('hidden')) {
...
async function fetchIssuedBooks(studentId) {
    try {
        const response = await fetch(`${API_BASE_URL}/students/${studentId}/issued-books`);
