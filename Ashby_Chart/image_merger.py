...
def radar_chart(df, properties, title):
    ...
    for i, material in enumerate(df["Material"]):
        values = (
            df.loc[df["Material"] == material, properties].values.flatten().tolist()
        )
...
radar_fig = radar_chart(
    radar_df,
    properties,
    "Material Properties for Aircraft De-icing Systems (Higher is Better)",
)
plt.tight_layout()
...
for prop, ax_idx in property_axes.items():
    ...
    for j, material in enumerate(df["Material"]):
        ...
        ax.bar(
            j,
            df.loc[df["Material"] == material, prop].values[0],
            color=color,
            alpha=0.7,
        )
...
for i, material in enumerate(df["Material"]):
    ...
    plt.scatter(
        df.loc[df["Material"] == material, "Corrosion Resistance"],
    ...
    # Add material label
    plt.text(
        df.loc[df["Material"] == material, "Corrosion Resistance"].values[0] + 0.05,
...
plt.title(
    "Thermal Conductivity vs. Corrosion Resistance for De-icing Materials", fontsize=16
)
...
def create_composite_image():
    ...
    try:
        ...
        image_files = {}
        for prefix in [
            "deicing_materials_heatmap",
            "deicing_materials_radar",
            "deicing_materials_properties",
            "deicing_material_ranking",
        ]:
            matching_files = [
                f
        ...
        fig.suptitle(
            "Material Selection Analysis for Aircraft De-icing Systems",
            fontsize=24,
            fontweight="bold",
            y=0.98,
        )
        ...
        composite_path = os.path.join(
            output_dir, f"deicing_material_analysis_composite_{timestamp}.png"
        )
        plt.savefig(composite_path, dpi=300, bbox_inches="tight")

        # Also save as PDF for better quality in publications
        pdf_path = os.path.join(
            output_dir, f"deicing_material_analysis_composite_{timestamp}.pdf"
        )
...
try:
    ...
    strengths = []
    if (
        df.loc[idx, "Thermal Conductivity (W/mK)"]
        > df["Thermal Conductivity (W/mK)"].median()
    ):
        strengths.append(
            f"Good thermal conductivity ({df.loc[idx, 'Thermal Conductivity (W/mK)']} W/mK)"
        )
    ...
    if df.loc[idx, "Corrosion Resistance"] >= 3:
        strengths.append(
            f"Moderate corrosion resistance (rated {df.loc[idx, 'Corrosion Resistance']} out of 5)"
        )
    if (
        df.loc[idx, "Relative Cost (arbitrary units)"]
        < df["Relative Cost (arbitrary units)"].median()
    ):
        strengths.append(
            f"Cost-effective (only {df.loc[idx, 'Relative Cost (arbitrary units)']} relative units)"
        )
    ...
    weaknesses = []
    if (
        df.loc[idx, "Electrical Resistivity (µΩ·m)"]
        > df["Electrical Resistivity (µΩ·m)"].median()
    ):
        weaknesses.append(
            f"Higher electrical resistivity ({df.loc[idx, 'Electrical Resistivity (µΩ·m)']} µΩ·m)"
        )
    if df.loc[idx, "Corrosion Resistance"] < 5:
        weaknesses.append(
            "May require coatings for improved resistance to de-icing fluids"
        )
    ...
    print("\nConclusion:")
    print(
        f"{highlighted_material} is the optimal choice for aircraft de-icing systems due to its balance of lightweight properties, corrosion resistance, and thermal conductivity. Its manufacturability and cost-effectiveness make it particularly suitable for this application."
    )
